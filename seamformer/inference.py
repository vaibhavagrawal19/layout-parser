...
import seamformer.utils_from_colab as pu
...
def postProcess_combined(scr, bin,thresh=40, rectangularKernel=7):
    ...
    for j in range(3):
        final_tmp[:, :, j] = tmp
    cv2.imwrite("final_tmp.jpg", final_tmp)
    scr = final_tmp
    bin[bin>=thresh]=255
    bin[bin<thresh]=0
    scr[scr>=thresh]=255
    ...
    if len(files_)>0:
        for f in files_ : 
            ...
            if 'Arabic' in args['model_name']:
                ...
                # cv2.imwrite(os.path.join(bin_folder,'bin_'+file_name),binaryMap)
                binaryMap = np.repeat(binaryMap[:, :, None], 3, axis=-1)
            ...
            # print(file_path)
            if args["mode"] == "visualize" or args["poly_vis"] == True:
                img2 = None
                if 'Arabic' in args['model_name']:
